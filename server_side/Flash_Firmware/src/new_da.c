...
int main(int argc, char** argv)
{
  ...
  ftruncate(shm_fd, N*num_Bytes);
  char *total_data_buf = mmap(0, N*num_Bytes, PROT_WRITE, MAP_SHARED, shm_fd, 0);
  if (total_data_buf == MAP_FAILED) {
      perror("mmap");
      exit(EXIT_FAILURE);
  }
    ...
    // printf("here?");
      ...
      if(mem_full>0)
      {
          ...
          if (cmd_buf == NULL || data_buf == NULL) {
            fprintf(stderr, "Error: cmd_buf or data_buf is NULL\n");
            break;
        }
          bcm2835_spi_transfernb(cmd_buf, data_buf, num_Bytes);
          if (numBytesRead + (num_Bytes - offset_bytes) >= N*num_Bytes) {
            fprintf(stderr, "Error: Attempting to write beyond allocated memory.\n");
...
//   printf("here?");
  ...
  munmap(total_data_buf, N*num_Bytes);
